...
## Input
Target Data: $ARGUMENTS (or {{args}})

## Workflow

### 1. Data Inspection & Discovery
- Use `ls -R` to locate relevant data files.
- Use `csvstat` or `vd` to get a high-level overview of CSV data.
- Identify the format (XBRL, CSV, JSON, XML, PDF).

### 2. Deep Parsing (XBRL)
If the data is in XBRL format:
- Use `arelleCmdLine` to parse the filing.
...
- **CSV**: Use `csvclean`, `csvcut`, and `csvgrep` to filter and normalize columns.
- **JSON**: Use `jq` to map raw fields to the internal RLM schema.
...
- Delegate to `@rlm-design` (or `/rlm-design`) once the data is ready for architectural planning.
