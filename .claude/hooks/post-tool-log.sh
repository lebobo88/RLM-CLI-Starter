#!/bin/bash
# RLM Post-Tool Log Hook (Claude Code) — tracks tool usage for progress reporting
...
if [ ! -f "$LOG_FILE" ]; then
  echo "timestamp,sessionId,tool,event" > "$LOG_FILE"
fi

NOW=$(date -Iseconds 2>/dev/null || date +%Y-%m-%dT%H:%M:%S%z)
echo "$NOW,$SESSION_ID,$TOOL_NAME,post_tool_use" >> "$LOG_FILE"
