...
3. **Parse user request**: Determine which sandbox operation from $ARGUMENTS
4. **Execute via sbx CLI**: All operations use `uv run sbx <command>` from the `sandbox/` directory. Provider is auto-detected; use `--provider docker` or `--provider e2b` to override
...
For implementation tasks in sandbox, delegate to `/rlm-implement`.
For verification tasks in sandbox, delegate to `/rlm-verify`.
