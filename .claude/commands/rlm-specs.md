...
If a specific feature was requested: $ARGUMENTS
...
Use the `@gemini-analyzer` subagent to verify that your proposed architectural decisions (ADRs) are compatible with the existing codebase and don't introduce regression patterns. Example: `@gemini-analyzer Compare my proposed data model in ADR-001 with the existing schema in src/db/schema.ts and identify conflicts.`
