#!/bin/bash
# RLM Session Start Hook (Gemini CLI) — logs session start and loads pipeline state
...
echo "$NOW | START | source=gemini-cli" >> "$LOG_DIR/sessions.log"
...
JSONL_ENTRY=$(jq -n --arg ts "$NOW" --arg src "gemini-cli" --arg sid "$SESSION_ID" --arg ph "$PHASE" \
